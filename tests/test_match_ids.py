...
class TestGetMatchIdsByPuuid:
    ...
    async def test_get_match_ids_by_puuid_basic(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
    ...
    async def test_get_match_ids_by_puuid_with_count(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
    ...
    async def test_get_match_ids_by_puuid_with_raw_queue_id(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
    ...
    async def test_get_match_ids_by_puuid_with_match_type(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
    ...
    async def test_get_match_ids_by_puuid_with_raw_match_type(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
        """Test get_match_ids_by_puuid with raw match type string."""
        result = await client.get_match_ids_by_puuid(
            test_puuid, match_type="normal", count=5
        )

        assert isinstance(result, list)

    async def test_get_match_ids_by_puuid_count_validation(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
    ...
    async def test_get_match_ids_by_puuid_pagination(
        self, client: "NexarClient", test_puuid: str
    ) -> None:
