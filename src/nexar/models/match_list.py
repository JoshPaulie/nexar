...
class MatchList(list["Match"]):
    ...
    def each_participant(self) -> Iterator["Participant"]:
        for match in self:
    ...
    def get_champion_stats(
        ...
        for participant in self.each_participant():
    ...
    def get_performance_stats(self) -> PerformanceStats:
        ...
        win_rate = wins / games_counted
        avg_kills = total_kills / games_counted
        avg_deaths = total_deaths / games_counted
        avg_assists = total_assists / games_counted
        avg_kda = (total_kills + total_assists) / total_deaths if total_deaths > 0 else 0.0
        avg_cs = total_cs / games_counted
        avg_game_duration_minutes = (total_duration / 60) / games_counted

        return PerformanceStats(
            total_games=games_counted,
            wins=wins,
            losses=losses,
            win_rate=win_rate,
            avg_kills=avg_kills,
    ...
    def get_average_stat(
        self,
        stat_retriever: Callable[["Participant"], int | float | None],
    ) -> float:
        """
        ...
        Args:
            stat_retriever: A lambda function that takes a Participant object
                            and returns the statistic to average. May return None
                            to skip a match (e.g. when an optional field is absent).

        Returns:
            The average value of the statistic, or 0.0 if no matches are found.

        Example:
            # Get average gold per minute (None-safe: absent fields skip a match)
        ...
        for participant in self.each_participant():
            value = stat_retriever(participant)
