...
match_ids = client.get_match_ids_by_puuid(puuid, queue=QueueId.RANKED_SOLO_5x5, count=3)
...
for match_num, match_id in enumerate(match_ids, 1):
    ...
    print(
        f"Match Duration: {match.info.game_duration // 60}m {match.info.game_duration % 60}s"
    )
    print(
        f"Result: {'WIN' if any(p.win for p in match.info.participants if p.team_id == player_team_id) else 'LOSS'}"
    )
    print()
        ...
        if participant.team_id == player_team_id:
            ...
            if participant.challenges:
                ...
                if c.damage_per_minute and match.info.game_duration > 0:
                    team_stats["total_damage"] += c.damage_per_minute * (
                        match.info.game_duration / 60
                    )
                if c.gold_per_minute and match.info.game_duration > 0:
                    team_stats["total_gold"] += c.gold_per_minute * (
                        match.info.game_duration / 60
                    )
                if c.vision_score_per_minute and match.info.game_duration > 0:
                    team_stats["vision_score"] += c.vision_score_per_minute * (
                        match.info.game_duration / 60
                    )
                ...
                if c.perfect_game:
                    team_stats["team_fight_performance"]["perfect_games"] += (
                        c.perfect_game
                    )
                if c.team_damage_percentage is not None:
                    team_stats["team_fight_performance"]["team_damage_share"].append(
                        c.team_damage_percentage
                    )
                if c.damage_taken_on_team_percentage is not None:
                    team_stats["team_fight_performance"]["damage_taken_share"].append(
                        c.damage_taken_on_team_percentage
                    )
        else:
    ...
    print(
        f"  Team KDA: {team_stats['total_kills']}/{team_stats['total_deaths']}/{team_stats['total_assists']}"
    )
    if team_stats["total_deaths"] > 0:
        team_kda = (
            team_stats["total_kills"] + team_stats["total_assists"]
        ) / team_stats["total_deaths"]
    ...
    if tf["team_damage_share"]:
        ...
        print(
            f"  Damage Distribution Balance: {max(damage_distribution) - min(damage_distribution):.1%} spread"
        )
    ...
    if tf["damage_taken_share"]:
        ...
        print(
            f"  Tank Distribution Balance: {max(tank_distribution) - min(tank_distribution):.1%} spread"
        )
    ...
    for participant in team_participants:
        ...
        if participant.challenges:
            ...
            if c.vision_score_per_minute and c.vision_score_per_minute > 2.5:
                highlights.append(
                    f"excellent vision ({c.vision_score_per_minute:.1f}/min)"
                )
        ...
        if highlights:
            print(
                f"  {participant.champion_name}{player_marker}: {', '.join(highlights)}"
            )
        else:
