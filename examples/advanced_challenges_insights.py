...
print(f"Win Rate: {performance_data['wins']}/{matches_count} ({performance_data['wins']/matches_count:.1%})")
print()

print("🏆 EPIC MONSTER MASTERY")
epic = performance_data["epic_monster_performance"]
print(f"  Baron Takedowns: {epic['baron_takedowns']} (avg: {epic['baron_takedowns']/matches_count:.1f}/game)")
print(f"  Dragon Takedowns: {epic['dragon_takedowns']} (avg: {epic['dragon_takedowns']/matches_count:.1f}/game)")
print(f"  Herald Takedowns: {epic['herald_takedowns']} (avg: {epic['herald_takedowns']/matches_count:.1f}/game)")
print(f"  Elder Dragon Participation: {epic['elder_dragon_kills']} times")
print()

print("👁️ VISION MASTERY")
vision = performance_data["vision_mastery"]
print(f"  Average Vision Score/Min: {vision['avg_vision_score_per_min']/matches_count:.1f}")
print(f"  Control Wards Placed: {vision['control_wards_placed']} (avg: {vision['control_wards_placed']/matches_count:.1f}/game)")
print(f"  Wards Destroyed: {vision['wards_destroyed']} (avg: {vision['wards_destroyed']/matches_count:.1f}/game)")
print(f"  Stealth Wards Placed: {vision['stealth_wards_placed']} (avg: {vision['stealth_wards_placed']/matches_count:.1f}/game)")
print()
...
print(f"  Turret Plates Taken: {macro['turret_plates_taken']} (avg: {macro['turret_plates_taken']/matches_count:.1f}/game)")
print(f"  Teleport-Assisted Takedowns: {macro['teleport_takedowns']}")
print(f"  Enemy Jungle Monsters: {macro['jungle_monsters_stolen']:.0f} (avg: {macro['jungle_monsters_stolen']/matches_count:.1f}/game)")
print()

print("📊 CONSISTENCY METRICS")
consistency = performance_data["consistency_metrics"]
print(f"  Average KDA: {consistency['avg_kda']/matches_count:.2f}")
print(f"  Average Kill Participation: {consistency['avg_kill_participation']/matches_count:.1%}")
print(f"  Average Damage/Min: {consistency['avg_damage_per_minute']/matches_count:.0f}")
...
print(f"  Games with Lane Advantage: {special['lane_opponent_advantages']}/{matches_count}")
print()
...
epic_total = epic['baron_takedowns'] + epic['dragon_takedowns'] + epic['herald_takedowns']
epic_score = min(epic_total / matches_count / 2.0 * 15, 15)  # Cap at 15 for 2+ epic/game
total_score += epic_score
max_score += 15

# Special achievements bonus (0-10 points)
special_total = (combat['multikills'] + combat['solo_kills'] + 
                combat['perfect_games'] * 3 + combat['flawless_aces'] * 2)
