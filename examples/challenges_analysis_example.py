...
match_ids = client.get_match_ids_by_puuid(puuid, queue=QueueId.RANKED_SOLO_5x5, count=5)
...
for i, match_id in enumerate(match_ids, 1):
    ...
    if challenges:
        ...
        if challenges.rift_herald_takedowns and challenges.rift_herald_takedowns > 0:
            epic_achievements.append(
                f"Herald kills: {challenges.rift_herald_takedowns}"
            )
        ...
        if challenges.control_wards_placed and challenges.control_wards_placed > 0:
            vision_achievements.append(
                f"Control wards: {challenges.control_wards_placed}"
            )
        if challenges.stealth_wards_placed and challenges.stealth_wards_placed > 0:
            vision_achievements.append(
                f"Stealth wards: {challenges.stealth_wards_placed}"
            )
        ...
        early_game = []
        if (
            challenges.takedowns_first_25_minutes
            and challenges.takedowns_first_25_minutes > 0
        ):
            early_game.append(
                f"Takedowns <25min: {challenges.takedowns_first_25_minutes}"
            )
        if challenges.first_turret_killed and challenges.first_turret_killed > 0:
            early_game.append("First turret!")
        if (
            challenges.lane_minions_first_10_minutes
            and challenges.lane_minions_first_10_minutes > 0
        ):
        ...
        if challenges.damage_taken_on_team_percentage is not None:
            print(
                f"Team Damage Taken: {challenges.damage_taken_on_team_percentage:.1%}"
            )
...
print(
    "This includes vision control, objective participation, early game impact, and much more."
)
